window.XiamiPlus = {
    "pluginFetch": function (url, trackList, callback) {
        //获取虾米的信息
        ...
        //精选集 /song/playlist/id/15427053/type/3

    },

    "fetchSongInfo" : function(song_id, callback){
        console.log('fetch xiami song info');
		var _this = this;
        $.get("http://www.xiami.com/song/"+song_id, function(resp){
			var song_list = [];
			var song = {};
			//song_id, name, album_id, album_name, album_logo, artist_id, artist_name
			song.song_id = song_id;
			song.name = /<meta property=\"og:title\" content=\"(.*?)\"\/>/.exec(resp)[1];
			if(song.name) song.name = $.trim(song.name);
			song.album_id = /<a class=\"CDcover185\" id=\"albumCover\" href=\"\/album\/(\d+)\"/.exec(resp)[1];
			song.album_name = /<meta property=\"og:music:album\" content=\"(.*?)\"\/>/.exec(resp)[1];
			song.album_logo = /<img class=\"cdCDcover185\" src=\"(.*?)\" \/>/.exec(resp)[1];
			song.artist_id = /<a href=\"\/artist\/(\d+)\"/.exec(resp)[1];
			song.artist_name = /<meta property=\"og:music:artist\" content=\"(.*?)\"\/>/.exec(resp)[1];
			var singerStr = /分享 (.*?) 的歌曲/.exec(resp)[1];
			song.singerStr = singerStr;
			song.singers = singerStr ? singerStr.split(';') : [];

			console.log(song);
			_this.searchTingWeibo(song, callback);
        });
    },

	"searchTingWeibo" : function(song, callback){
		//{"code":1,
		// "data":[
		//			{
		//				"object_id":"1022:10151501_336614",
		//				"songname":"u4eca\u5929\u4f60\u8981\u5ac1\u7ed9\u6211 Marry Me Today",
		//				"singers":[
		//						{
		//							"singer_name":"\u8521\u4f9d\u6797",
		//							"singer_url":"http:\/\/weibo.com\/1742727537?from=page_101515_player"
		//						},
		//						{
		//							"singer_name":"\u9676\u5586",
		//							"singer_url":"http:\/\/weibo.com\/1630856882?from=page_101515_player"
		//							}
		//					],
		//				"is_like":false,
		//				"copy":"",
		...
		//				"mp3_url":"http:\/\/ting.weibo.com\/page\/appclient\/getmp3?uid=1749149061&object_id=336614&source_id=1040&coflag=native_play_list&sig=0f45f2&plat=0",
		//				"play_length":"311",
		//				"page_url":"http:\/\/weibo.com\/p\/10151501_336614?from=page_101515_player&source_id=1040",
		//				"source_link":"http:\/\/www.xiami.com?from=page_101515_player",
		//				"source_id":"1040",
		//				"is_dashang":0
		//			}
		//		]
...
KISSY.add('utils/index/global',['node'], function(S, require, exports, module) {
    ...
    var global = {
        ...
        //批量推广的下载单曲
        promotion_downloads :  function(ids, type, pid){
 ...
 * @zhongtang
 */
KISSY.add('page/mods/player/player-ad',['node', 'base'], function(S, require, exports, module){
    var Node = require("node"),
        Base = require("base");

    var $ = S.all;

    var playerAdExtension = {

        /**
         * audioAdInit 音频广告数据封装（歌曲形式及文本格式)
         * @param  {Object} data 音频广告数据格式
         */
        audioAdInit:function(data){

            var self = this;
            var adObj = {};
            adObj.atPlay = 2;
            adObj.clearlist = null;
            adObj.destroyed = false;
            adObj.host =  "";
            adObj.hqset = "0";
            adObj.lastSongId = data.artistAdId;
            adObj.adId = data.artistAdId;
            adObj.lastSongToggle = true;
            adObj.listeners = [];
            adObj.plugins = [];
            adObj.type = "default";
            adObj.typeId =  1;
            ...
            adObj.vip = 0;
            adObj.vipRole = 2;
            adObj.adId = data.adId;
            adObj.adDid = data.adDid;
            adObj.trackList = [{
                isAd:true,
                title:data.title,
                song_id:data.artistAdId,
                location:data.ad_mp3_secret,
                artist_id:data.artist_id,
                grade: -1,
                insert_type: "1",
                object_id: data.artistAdId,
                object_name: "default",
                tryhq: 0,
                rec_note: "",
                artist:data.artist_name,
                album_id: data.artistAdId,
                album_name: data.artist_name,
                album_pic: data.ad_img_url,
                aritst_type: "",
                artist_url: data.target_url,
                adUrl:data.target_url,
                background: "",
                length: "",
                lyric: "audioad",
                lyric_url: "",
                ms: null,
                pic: data.ad_img_url,
                textAdCont: data.content
            }];

            if(adObj.trackList.length > 0 ) {
                self.set('adParamArr',adObj.trackList);
                self.fire("audioAdComplete",{
                    data:adObj
                });
                self.set("adIsload",true);

                var newArtistIdArr = self.get("artistIdArr") ? self.get("artistIdArr") : [];

                newArtistIdArr.push(data.artist_id)
                self.set("artistIdArr",newArtistIdArr);
            }
        },
        /**
         * renderSidebarAd 渲染广告封面
         * @param {Array} arr 当前播放歌曲数据
         */
        renderSideBarAd:function(data){
            var self = this;
            var tpl = '<div><h3 style="text-align:center;"><a href="'+data[0]['adUrl']+'" target="_blank" style="text-align:center;font-size:12px;color:#2f63cf;font-family:Microsoft Yahei;text-decoration:none">'+data[0]['title']+'</a></h3><a style="display:block;margin:15px auto;text-align:center;" href="'+data[0]['adUrl']+'" target="_blank"><img src="http://gtms03.alicdn.com/tps/i3/TB1eNyeGXXXXXb7XpXX9Y8fHXXX-170-39.png" /></a>'+data[0]['textAdCont']+'</div>';
            $("#J_playerLrc").html(tpl);
        }
    ...
    module.exports = Base.extend(playerAdExtension,playerAdAttrs);
...
KISSY.add('page/mods/xtpl/lrc-xtpl',function (S, require, exports, module) {
    ...
    return function (scope, S, undefined) {
        ...
        config0.fn = function (scope) {
            ...
            var config3 = {};
            var params4 = [];
            var id5 = getPropertyUtil(engine, scope, "info", 0, 3);
            params4.push(id5);
            config3.params = params4;
            config3.fn = function (scope) {
                var buffer = "";
                buffer += '<li class="ui-lrc-info">';
                var id6 = getPropertyOrRunCommandUtil(engine, scope, {}, "info", 0, 3);
            ...
            var config7 = {};
            var params8 = [];
            var id9 = getPropertyUtil(engine, scope, "data", 0, 4);
            params8.push(id9);
            ...
            config7.fn = function (scope) {
                ...
                var config10 = {};
                var params11 = [];
                var id12 = getPropertyUtil(engine, scope, "xindex", 0, 5);
                params11.push(id12 === (0));
                config10.params = params11;
                config10.fn = function (scope) {
                    var buffer = "";
                    buffer += '\r\n<li class="ui-lrc-line ui-lrc-current">';
                    var id13 = getPropertyOrRunCommandUtil(engine, scope, {}, "text", 0, 6);
                ...
                config10.inverse = function (scope) {
                    ...
                    var id14 = getPropertyOrRunCommandUtil(engine, scope, {}, "text", 0, 8);
                    buffer += renderOutputUtil(id14, false);
                    buffer += '</li>\r\n';
                    return buffer;
                };
                buffer += runBlockCommandUtil(engine, scope, config10, "if", 5);
                buffer += '\r\n';
                return buffer;
            };
            buffer += runBlockCommandUtil(engine, scope, config7, "each", 4);
            buffer += '\r\n</ul>\r\n';
            return buffer;
        };
        config0.inverse = function (scope) {
            ...
            var id15 = getPropertyOrRunCommandUtil(engine, scope, {}, "id", 0, 13);
...
/** Compiled By kissy-xtemplate */
KISSY.add('page/mods/xtpl/pureMusic-xtpl',function (S, require, exports, module) {
    /*jshint quotmark:false, loopfunc:true, indent:false, asi:true, unused:false, boss:true*/
    return function (scope, S, undefined) {
        var buffer = "",
            config = this.config,
        ...
        var runBlockCommandUtil = utils.runBlockCommand,
            ...
            runInlineCommandUtil = utils.runInlineCommand,
            getPropertyOrRunCommandUtil = utils.getPropertyOrRunCommand;
        buffer += '<div class="pure-music">纯音乐曲目，敬请欣赏</div>\r\n';
        return buffer;
    };
});
KISSY.add('page/mods/player/player-lrc',['node', 'base', 'io', 'xtemplate', 'anim', 'utils/scrollView/scrollViewManage', '../xtpl/lrc-xtpl', '../xtpl/lrcText-xtpl', '../xtpl/pureMusic-xtpl'], function(S, require, exports, module) {
    // @formatter:off
    var Node = require("node"),
        Base = require("base"),
        IO = require("io"),
    ...
    PureMusicTpl = require("../xtpl/pureMusic-xtpl");
    ...
    var r1              = /\[(\d\d\:\d\d)\]/gm;
    var r2              = /\[(\d\:\d\d)\]/gm;
    var reg_soundname   = /\[ti:(.+?)\]/i;    //歌曲名
    var reg_singer      = /\[ar:(.+?)\]/i;    //演唱者
    var reg_words       = /\[lr:(.+?)\]/i;    //作词人
    var reg_composer    = /\[co:(.+?)\]/i;    //作曲人
    var reg_arranger    = /\[ag:(.+?)\]/i;    //编曲人
    var reg_CD          = /\[al:(.+?)\]/i;    //专辑
    var reg_maker       = /\[(by:.+?)\]/i;    //歌词作者
    var reg_offset      = /\[offset:(.+?)\]/i;    //偏移值
    var reg_take        = /^(\[\d\d\:\d\d(\.\d{1,3})?\])+[^\[\]\n]*/gm;    //获取歌词，去除歌曲信息
    var reg_replacetime = /(\[\d\d:\d\d\.\d{2,3}\])+/g;    //过滤时间信息的正则
    var reg_gettimes    = /(\[\d\d:\d\d\.\d{2,3}\])/g;    //获取时间信息的正则
    var reg_readtime    = /\[(\d\d):(\d\d\.\d{2,3})\]/;
    var reg_dis         = /^\s*$/;    //均衡歌词间距的正则
    ...
    var LrcExtension = {
        initializer: function() {
            ...
            self.TPL_pureMusic = new Xtemplate(PureMusicTpl);
            self.wrap = self.get("wrap");
            self.scrollView = null;
            self._hasLyric = false;
            self._offsetNum = 0;
            self.LyricsArr = [];
            self.LyricsInfo = null;
            self.LyricLine = null;
        ...
        lyricLoadComplete: function(value) {
            ...
            if (value != "") {
                ...
                if (!(/\[(\d+)\:(\d+)(\.\d+)?\]/.test(value))) {
                    ...
                    self._hasLyric = true;
                    self.LyricsArr = self._splitLyric(value).lrc;
                    self.LyricsInfo = self._splitLyric(value).info
                    self._show(true, self.LyricsArr, self.LyricsInfo);
                }
            } else {
                self.reset();
            }
        ...
        pureMusic: function() {
            var self = this;
            self.clearLyric();
            var html = self.TPL_pureMusic.render({});
            self.wrap.html(html);
            self.LyricLine = self.wrap.all(".ui-lrc-line");
            self.scrollView = ScrollViewManage.render("J_lyricScrollView");
        },
        ...
        _splitLyric: function(value) {
            ...
            // 获取对应内容
            // var arr_soundname = lrcData.match(reg_soundname);        // 歌曲名字
            // var arr_singer    = lrcData.match(reg_singer);           // 演唱者
            // var arr_words     = lrcData.match(reg_words);            // 作词人
            // var arr_composer  = lrcData.match(reg_composer);         // 作曲人
            // var arr_arranger  = lrcData.match(reg_arranger);         // 编曲人
            ...
            //   info += arr_soundname[1] + "<br/>";
            // };
            // if (arr_singer != null) {
            //   info += "演唱者 : " + arr_singer[1] + "<br/>";
            // };
            // if (arr_words != null) {
            //   info += "作词人 : " + arr_words[1] + "<br/>";
            // };
            // if (arr_composer != null) {
            //   info += "作曲人 : " + arr_composer[1] + "<br/>";
            // };
            // if (arr_arranger != null) {
            //   info += "编曲人 : " + arr_arranger[1];
            // };
            // if (arr_maker != null) {
            //   info += arr_maker[1] + "\r";
            // };
            ...
            // _arr_splitedLyrics.unshift({'text': info, 'time': 0})
            return {
                'lrc' : _arr_splitedLyrics,
                'info' : info
            };
        },
...
KISSY.add('utils/base',function(S, COOKIE) {
    ...
    var SERVER_HOST = 'http://www.xiami.com';
    //@zhongtang 音频广告地址
...
KISSY.add('page/mods/player/player-tracks',['node', 'base', 'anim', 'xtemplate', '../xtpl/trackItem-xtpl', 'utils/scrollView/scrollViewManage'], function(S, require, exports, module) {
    ...
    var playerTracksExtension = {
        ...
        syncScrollViewPosition: function(index) {
            ...
            var viewScrollTop;
            try{
                viewScrollTop = self.scrollView.get('scrollTop')
            }catch(e){
                viewScrollTop = self.scrollView.scrollTop()
            }
            var top = 41 * (index - 0);
...
KISSY.add('page/mods/player/player-data',['base', 'json', 'io', 'utils/base', './player-anim'], function(S, require, exports, module) {
    ...
    var playerDataExtension = {
        ...
        /**
         * @zhongtang
         * getAdArr 获取含有广告的艺人ID 数组格式
         */
        // getAdArr:function(nowAid){
        //     var self = this;

        //     new IO({
        //         url: BaseConfig.AD_ARTISTIDARR_URL,
        //         dataType: "jsonp",
        //         success: function(res) {
        //             // debugger;
        //             if (res.code === 0 && res.data) {
        //                 var artistIdObj = {};
        //                 artistIdObj.artistid = nowAid;
        //                 artistIdObj.artistAdIdArr = res.data;
        //                 self.fire('getPlayerAd',{
        //                     data:artistIdObj
        //                 });
        ...
        /**
         * zhongtang
         * getAdMsg 根据艺人ID获取相关广告信息
         * @param  {String} aid 艺人ID
         * @param {String} [uid] [用户ID]
         */
        getAdMsg:function(aid,uid){
            var self = this;

            new IO({
                url: BaseConfig.AD_ARTIST_MSG_URL,
                data:{artistId:aid,uid:uid},
                dataType: "jsonp",
                success: function(res) {

                    if (res.code === 0) {
                        self._addPlayerAdFn(res.data);
                    }
                },
                error: function() {
                    return;
        ...
        getUserAd:function(uid){
            var self = this;

            new IO({
                url:BaseConfig.AD_USER_URL,
                data:{uid:uid},
                dataType:"jsonp",
                success:function(res){
                    if(res.code === 0) {
                        self.set("isPlayUserAd",true);
                        self.set('adIoLoaded',true);
                        self._addPlayerAdFn(res.data);
                    }else {
                        self.set('adIoLoaded',true);
                    }
        ...
        sendAdPlayLog:function(id,did,uid){
            var self = this;
            new IO({
                url:BaseConfig.AD_PLAYLOG,
                data:{id:id,did:did,uid:uid,event:"play"},
                dataType:"jsonp",
                success:function(res){
                }
            });
        },
        _addPlayerAdFn:function(data){
            var self = this;
            var sid = (new Date().getTime()).toString();
            self.set('artistAdId',sid);
            //self.set('adIsLoad',true);
            self.set('artistAdCont',data.content);
            self.set('adId',data.id);
            self.set('adDid',data.did);
            self.fire('addPlayerAd',{
                'data':data
            });
        },
        /********************************共用方法 ***********************/
...
KISSY.add('page/mods/xtpl/pageItem-xtpl',function (S, require, exports, module) {
    ...
    return function (scope, S, undefined) {
        ...
        config0.fn = function (scope) {
            ...
            buffer += renderOutputUtil(id6, true);
            buffer += ')"><i class="icon-thenplay"></i>接着播放</a></li>\r\n\t\t<li><a href="javascript:void(0)" onclick="SEIYA.promotion_downloads(\'';
...
KISSY.add('page/mods/player/player-cover',['io', 'xtemplate', 'event', 'node'], function(S, require) {
    ...
    cover.prototype = {
        render: function(track) {
            ...
            /**
             * 专辑ID必须大于0 #bug
             * @time 2014-10-29 14:15:56
             */
            if (self.track && track.albumId > 0 && track.albumId == self.track.albumId) {
                return true;
            };
            self.track = track;
            ...
            new IO({
                ...
                success: function(responres) {
                    if (responres.status) {
						...
						//hack it
                        console.log('got in: data center');
                        XiamiPlus.pluginFetch(url, responres.data.trackList, function(trackList){
								console.log('send into player', trackList);
								self.set('trackList', trackList);
								self.fire('complete', {
									'status' : true
								});
						});
 ...
 * @mail nongyoubao@alibaba-inc.com
 */
KISSY.add('page/mods/player',['node', 'base', 'json', 'event', 'io', 'xtemplate', './player/player-swfobj', './player/player-ad', './player/player-lrc', './player/player-sale', './player/player-lister', './player/player-volume', './player/player-panel', './player/player-control', './player/player-tracks', './player/player-blur', './player/player-data', './player/player-menu', './player/player-roam', './player/player-log', './player/player-drag', './player/player-cover', './player/player-switch', './data/center', './xtpl/trackInfo-xtpl', 'widget/tool/index', 'utils/tip/index', './user', 'utils/goldlog/index'], function(S, require, exports, module) {
    // @formatter:off
    ...
    //PlayerSwf = require("./player/player-swf"),
        ...
        PlayerAd = require("./player/player-ad"),
    ...
    player.prototype = {
        ...
        setMusicInfo: function(json, passtime) {
            ...
            var data = {
                data: [track],
                artistfun: function() {
                    var artist = this.artist ? S.unEscapeHTML(this.artist) : "AD";
                    var audioad = S.unEscapeHTML(this.lyric);
                    var arr = artist.split(";");
                    result = [];
                    if (arr.length == 1 && audioad !== "audioad") {
                        return '<a href="' + this.artistUrl + '" target="_blank" title="' + artist + '">' + artist + '</a>';
                    } else {
                        if (arr[0] === "AD") {
                            return '<span title="' + artist + '">' + artist + '</span>';;
                        } else {
                            return '<a href="http://www.xiami.com/artist/' + this.artistId + '" target="_blank" title="' + artist + '">' + artist + '</a>';
                        }
            ...
            if (track.lyric === "audioad") {
                ...
                self.PlayerAd.renderSideBarAd(self.PlayerAd.get('adParamArr'));
                self.PlayerData.sendAdPlayLog(self.PlayerData.get('adId'), self.PlayerData.get('adDid'), self.USER.get('uid'));
            ...
            //获取相对应艺人广告信息 @zhongtang
            //只在播放模式下
            if (self.PlayerData.get("status") == "play") {

                if (!self.PlayerData.get("adIoLoaded")) {
                    self.PlayerData.getUserAd(self.USER.get("uid"));
                }
                self._isPlayedAd(track);
            }
        ...
        _playerAd: function() {
            var self = this;
            self.PlayerAd = new PlayerAd();
            //注册音频广告加载事件
            self.PlayerAd.on("audioAdComplete", function(event) {
                var songId = self.PlayerData.get('songId'); //当前播放歌曲的ID
                var data = event.data;
                self.PlayerTracks.add(data.trackList, [], songId);
                self._dataCenterCompleteHandler(data);

            });

            // self.PlayerAd.on("textAdComplate",function(event){
            //     self.PlayerLrc.textAdComplate(event.data);
            // });

        },
        /**
         *  @zhongtang
         * _playerAdMsg 通过当前播放歌曲的艺人ID进行匹配
         * @param  {Number} artistid 当前播放歌曲的艺人ID
         */
        // _playerAdMsg: function(data) {
        //     var self = this;
        //     var artistAdObj = data;
        //     var uid = self.USER.get('uid') ? self.USER.get('uid') : "";
        //     if (artistAdObj.artistAdIdArr.length != 0 && artistAdObj.artistid) {
        //         S.each(artistAdObj.artistAdIdArr, function(n, i) {
        //             if (n === artistAdObj.artistid) {
        //                 //self.PlayerData.getAdMsg(artistAdObj.artistid,new Date().getTime());
        //                 self.PlayerData.getAdMsg(artistAdObj.artistid,uid);
        //             }
        //         });
        //     }
        ...
        /**
         * @zhongtang
         * _playerAdInit 渲染音频广告
         * @param  {Object} data 音频广告数据
         * @param  {Number} aid  艺人ID
         */
        _playerAdInit: function(data, aid) {
            var self = this;
            self.PlayerAd.audioAdInit(data);
        },

        /**
         * @zhongtang
         * [_isPlayedAd 检查是否播放过广告(用户定位广告投放)]
         * @param  {Object}  track 歌曲播放信息
         */
        _isPlayedAd: function(track) {
            var self = this;
            if (!self.PlayerData.get("adIoLoaded")) {
            ...
            if (!self.PlayerData.get("isPlayUserAd") && track.lyric !== "audioad") {
                //self.PlayerData.getAdArr(track.artistId);
                var artistIdArr = self.PlayerAd.get("artistIdArr");
                if (artistIdArr && artistIdArr.length > 0) {
                    if (!addKey(artistIdArr, track.artistId)) {
                        self.PlayerData.getAdMsg(track.artistId, self.USER.get('uid'));
                    }
                } else {
                    self.PlayerData.getAdMsg(track.artistId, self.USER.get('uid'));
                }
            ...
            function addKey(arr, key) {
                ...
                if (arrs.indexOf(key) >= 0) { // 在字符串arrs中查找key第一次出现的位置, 如果不存在则返回值为-1
                    return true;
                } else {
                    return false;
                }
...
KISSY.add('page/mods/main',['node', 'io', 'event', 'xtemplate', 'utils/index/global', './shortcut', './player', './sidebar', './page', './user', './search'], function(S, require, exports, module) {
    ...
    pt.init = function() {
        ...
        var swfurl = location.href.indexOf("swf-debug") !== -1 ? "http://gitlabswf.xiami.com/music/xiamiplayer/1.5/player.swf" : "http://img.xiami.net/static/swf/seiya/1.5/player.swf";
